...
daemon читает серверный `config.json` только на старте, поэтому `setup` сам останавливает
работающий daemon после правки — следующий вызов агента поднимет новый с актуальным конфигом.
Если daemon почему-то не остановился, останови его вручную (`pkill -f -- --daemon`). Дополнительно:
смена набора порталов (`setup` → add/remove/edit) переписывает `manifest.json` → **перезагрузи
расширение** в `chrome://extensions`; смена порта/токена меняет `config.json` расширения →
**переоткрой вкладку** портала.
